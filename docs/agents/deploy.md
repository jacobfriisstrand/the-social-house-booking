...
Pull requests targeting `develop` or `main` also run a release preflight against the matching GitHub Environment. It uses that environment's secrets to run `supabase db push --dry-run` and `supabase config diff`, catching invalid project access, database credentials, pending migration errors, and configuration errors before merge. Supabase does not provide a dry-run mode for function deployment or Resend template synchronization, so those steps remain covered by the post-merge release workflow.
