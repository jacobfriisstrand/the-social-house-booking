...
- Mapped action types: `invite` → `company-invitation` (#26), `recovery` → `password-reset` (#70). Every other action type, and any alias without a registry entry, is answered with a 4xx, which makes Auth fail the call instead of sending a blank mail. Member email changes use the app's two-step approval flow and do not fire `email_change`.
- The link is `<redirect_to>/set-password?token_hash=<hash>&type=<action>`, where `email_data.redirect_to` is what Auth fills from the project's `site_url` in `config.toml` (`[remotes.<name>.auth]`) when the invite passes no redirect. Do not use `email_data.site_url`: Auth puts its own API URL there, not the site (found end to end on 2026-09-14). The page verifies the token with `verifyOtp` (#1).
...
- Locally the hook is disabled in the base `config.toml`, so `supabase start` delivers invites to the mail catcher on port 54324. To exercise the function itself, put `APP_ENV`, `EMAIL_REDIRECT_TO`, `RESEND_API_KEY`, `RESEND_FROM` and `SEND_EMAIL_HOOK_SECRET` in `supabase/functions/.env` (gitignored) and run `supabase functions serve send-email`; serving mechanics and the signed simulated hook call are in `supabase.md` (Edge Functions). In the cloud the same five are Edge Function secrets (`docs/agents/deploy.md`); `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are injected.
