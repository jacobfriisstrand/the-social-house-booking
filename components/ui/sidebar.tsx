...
// The collapse state cookie; components/shell/sidebar-cookie.ts reads it on
// the server so a collapsed sidebar does not flash open (issue #55).
      ...
      <Sheet onOpenChange={setOpenMobile} open={openMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{messages.shell.menuSheetTitle}</SheetTitle>
            <SheetDescription>
              {messages.shell.menuSheetDescription}
            </SheetDescription>
          </SheetHeader>
    ...
    >
      ...
      <div
        className={cn(
          ...
          // Adjust the padding for floating and inset variants. The sidebar
          // variant has no border: the sidebar is white like the page, and
          // the muted panel separates content from chrome (DESIGN.md).
          variant === "floating" || variant === "inset"
        ...
        {...props}
      >
        {/* This is what gives the Sidebar its name */}
        <div
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn("flex flex-col gap-2 p-2 pt-7", className)}
      data-sidebar="header"
...
function SidebarGroupLabel({
  ...
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex h-8 shrink-0 items-center rounded-md px-2 font-medium text-sidebar-foreground/70 text-xs outline-hidden ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 group-data-[collapsible=icon]:opacity-0 [&>svg]:size-4 [&>svg]:shrink-0",
          className
        ),
      },
      props
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button group-data-[collapsible=icon]:justify-center! flex w-full cursor-pointer items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:h-8! group-data-[collapsible=icon]:p-0! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    defaultVariants: {
      size: "default",
      variant: "default",
