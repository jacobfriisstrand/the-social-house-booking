...
_How to customize and configure Biome with biome.json._

(Content from the biomejs/website source file `src/content/docs/en/reference/configuration.mdx`, which is the source of the rendered page.)
...
A list of [GritQL plugins](https://biomejs.dev/linter/plugins/) to enable. Each entry can be
...
A list of [glob patterns](#glob-syntax-reference).
...
While crawling the file system, Biome follows symbolic-link chains up to three
links deep. Longer chains are skipped and produce a warning diagnostic.
...
> Please use the
> [_force-ignore syntax_](https://biomejs.dev/reference/configuration/#interaction-with-the-scanner)
...
specified in the VCS ignore files, Git's local exclude file, and `.ignore` files.
...
to the VCS root here. The path must be relative to the configuration file. This ensures that Biome
can correctly find and follow your VCS ignore files.

For example, if your Biome configuration is located at `frontend/biome.json` and your VCS root is at the project root, you should configure `vcs.root` like this:
...
> **Caution**
>
> This options is deprecated. Use [`linter.rules.preset`](https://biomejs.dev/reference/configuration#lintrulespreset) instead.

### `linter.rules.preset`

It allows enabling a certain set of specific rules. It accepts the following values:
- `"recommended"`: it enables Biome's recommended rules (default)
- `"all"`: it enables all lint rules, except for nursery rules.
- `"none"`: it disables all lint rules.

### `linter.rules.[group]`

Options that influence the rules of a single group. Biome supports the following groups:

- a11y: Rules focused on preventing accessibility problems.
...
- `"auto"`, matches the operating system's expected line-ending style: CRLF (`\r\n`) on Windows, LF (`\n`) on macOS/Linux.
...
Enables support for resolving dependencies from default and named pnpm catalogs.

When enabled, Biome can use `catalog` and `catalogs` entries in `pnpm-workspace.yaml` while resolving package dependencies.
...
Enables the parsing of Vue-specific syntax inside `.html` files. Enabling this option also enables `html.parser.interpolation` implicitly, since it's a core part of Vue's syntax.

Biome already auto detects `.vue` files, so you likely don't need to enable this option unless your project uses `.html` files for Vue components.
