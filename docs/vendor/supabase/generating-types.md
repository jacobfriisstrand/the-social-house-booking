...
or in case of a self-hosted instance (see [Accessing Postgres](https://supabase.com/docs/guides/self-hosting/accessing-postgres#connect-through-supavisor) for more information):
...
Note: To use `MergeDeep`, set `compilerOptions.strictNullChecks` to `true` in your `tsconfig.json`.
